...
public class UpdateSingleCommandHandler(
    ...
    IUnitOfWork unitOfWork) : ICommandHandler<UpdateEPCommand>
{
    public async Task<Result> Handle(UpdateEPCommand request, CancellationToken token)
    {
        ...
        var playlist = await playlistRepository.GetReleaseByIdAsync(request.EPId, token);
