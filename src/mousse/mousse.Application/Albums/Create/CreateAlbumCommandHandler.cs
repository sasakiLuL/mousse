...
public class CreateEPCommandHandler(
    IUserRepository userRepository,
    IUserIdentityProvider userIdentityProvider,
    IPlaylistRepository playlistRepository,
    ITagRepository tagRepository,
    IUnitOfWork unitOfWork) : ICommandHandler<CreateAlbumCommand>
{
    public async Task<Result> Handle(CreateAlbumCommand request, CancellationToken token)
    {
        ...
        var playlistNameResult = PlaylistName.Create(request.AlbumName);
        ...
        {
            releasesResults.Add(await CreateTrackFromRequestAsync(r, albumId, token));
        }
        ...
        await playlistRepository.AddAsync(albumResult.Value, token);
    ...
    private async Task<Result<Track>> CreateTrackFromRequestAsync(
        TrackRequest trackRequest, Guid albumId, CancellationToken token = default)
    {
