...
public class CreateSingleCommandHandler(
    ...
    IUnitOfWork unitOfWork) : ICommandHandler<CreateSingleCommand>
{
    public async Task<Result> Handle(CreateSingleCommand request, CancellationToken token)
    {
        ...
        {
            releasesResults.Add(await CreateTrackFromRequestAsync(r, singleId, token));
        }
        ...
        var singleResult = Single.Create(
                singleId,
        ...
        await playlistRepository.AddAsync(singleResult.Value, token);
    ...
    private async Task<Result<Track>> CreateTrackFromRequestAsync(
        TrackRequest trackRequest, Guid singleId, CancellationToken token = default)
    {
